...
import { X, Search, Filter, Eye, Check, DollarSign } from 'lucide-react';
...
interface OfferSelectorModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSelectOffer: (commission: Commission, offerPrice: number, offerComments: string) => void;
...
export default function OfferSelectorModal({ 
  ...
  // Update offer price when commission is selected
  useEffect(() => {
    if (selectedCommission) {
      setOfferPrice(selectedCommission.proposedAmount.toFixed(2));
    }
  }, [selectedCommission]);
  ...
  const handleCreateOffer = () => {
    if (selectedCommission && offerPrice) {
      const price = parseFloat(offerPrice);
      if (!isNaN(price) && price > 0) {
        onSelectOffer(selectedCommission, price, offerComments);
        onClose();
      }
    }
  };
  ...
  // Helper component to render a commission card
      ...
      <div className="flex items-start justify-between mb-3">
        <div className="flex-1">
          <div className="flex items-center space-x-2 mb-2">
            ...
            {user?.isAdmin && commission.userId !== user.id && (
          ...
          <p className="text-amber-400 text-xs font-mono">{commission.referenceNumber}</p>
        </div>
      ...
      {commission.tags && commission.tags.length > 0 && (
        <div className="flex flex-wrap gap-1 mb-3">
          {commission.tags.slice(0, 3).map((tag, idx) => (
            <span
              key={idx}
              className="bg-amber-500/20 text-amber-300 border border-amber-500/30 px-2 py-1 rounded text-xs font-medium"
            >
              {tag}
            </span>
          ))}
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 px-4 animate-fade-in">
      <div className="bg-gray-900 rounded-xl max-w-4xl w-full max-h-[90vh] flex flex-col border border-amber-500/20 animate-scale-in">
        {/* Header */}
        <div className="p-6 border-b border-amber-500/20 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-400 mt-1">
              {user?.isAdmin 
                ? "Admins can create offers for any commission from conversation participants"
                : "You can only create offers for your own commissions"
              }
            </p>
          </div>
          <button
        ...
        <div className="p-6 border-b border-amber-500/20">
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full bg-gray-800 border border-amber-500/20 text-white pl-10 pr-4 py-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500 placeholder-gray-400"
              />
            </div>
            ...
            <button
              ...
              className="px-4 py-3 bg-amber-600 hover:bg-amber-500 text-white rounded-lg transition-colors flex items-center space-x-2"
            >
          ...
          {/* Filter Dropdown */}
          {showFilters && (
            <div className="mt-4 p-4 bg-gray-800 rounded-lg border border-amber-500/20">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-gray-700 border border-gray-600 text-white px-3 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500"
                  >
          ...
          {isLoadingCommissions ? (
            ...
            <div className="space-y-6">
              ...
              {(user?.isAdmin ? myCommissions.length > 0 : myCommissions.length > 0) && (
                <div>
                  {user?.isAdmin && (
                    <h3 className="text-lg font-semibold text-white mb-3 pb-2 border-b border-amber-500/20">
                      My Commissions
                    </h3>
                  )}
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {myCommissions.map(renderCommissionCard)}
                  </div>
                </div>
              )}
              ...
              {user?.isAdmin && otherUsersCommissions.length > 0 && (
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {otherUsersCommissions.map(renderCommissionCard)}
                  </div>
                </div>
              )}
            </div>
        ...
        {selectedCommission && (
          <div className="p-6 border-t border-amber-500/20 bg-gray-800/50">
            <h3 className="text-lg font-bold text-white mb-4">Offer Details</h3>
            ...
            <div className="mb-4">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-gray-700 border border-amber-500/20 text-white pl-10 pr-4 py-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500"
                  min="0"
                  step="0.01"
                />
              </div>
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full bg-gray-700 border border-amber-500/20 text-white px-4 py-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500 resize-none"
                rows={3}
              />
            </div>
          </div>
        ...
        {/* Footer */}
        <div className="p-6 border-t border-amber-500/20 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-3">
            ...
            <button
              ...
              disabled={!selectedCommission || !offerPrice || parseFloat(offerPrice) <= 0}
              className="px-6 py-2 bg-amber-600 hover:bg-amber-500 disabled:bg-gray-600 disabled:cursor-not-allowed text-white rounded-lg transition-colors flex items-center space-x-2"
            >
